...
def get_metric(metric_num):
    ...
    metrics = ("pfIssued", "pfInCache", "overallMisses", "overallHits", "overallAccesses", "unusedPrefetches",
               "overallMisses overallAcceses", "overallHits overallAccesses", "unusedPrefetches pfIssued", "pfInCache unusedPrefetches")
...
def graph(title, workload, prefetchers, x_data_set, y_data_set, x_min, x_max):
    ...
    for i in range(len(prefetchers)):
        ...
        marker_pos += 1
        x_data = array_thinnerX(x_data_set[i])
        y_data = array_thinnerY(y_data_set[i])
...
def parse_input(args):
    ...
    graph(title, workload, prefetchers, x_data, y_data, xmin, xmax)
